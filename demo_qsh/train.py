...
def relation_loss(output, target, target_label, alpha_relation):
    ...
    log_num=torch.where(output<0.001,torch.tensor(0.001).double(),output)
    log_num2=torch.where(output>0.999,torch.tensor(0.999).double(),output)
...
def train():
    ...
    for epoch in range(opt.max_epoch):
        ...
        for (input, target, target2) in t:
            ...
            '''
            predicted3 = torch.where(predicted2.int()==target2.int(), torch.tensor(1), torch.tensor(0))
            predicted3 = torch.sum(predicted3, dim=2)
            predicted3 = torch.where(predicted3.int()==data.num_relation_classes, torch.tensor(1), torch.tensor(0))
            target3 = torch.where(predicted3==predicted3, torch.tensor(1), torch.tensor(0))
            '''
            predicted3 = torch.where(predicted2.int()==target2.int(), torch.tensor(1), torch.tensor(0))
            predicted3 = torch.sum(predicted3, dim=2)
            predicted3 = torch.where(predicted3.int()==data.num_relation_classes, torch.tensor(1), torch.tensor(0))
            ...
            '''
            print('---------------')
            print(predicted3.view(-1).shape)
            print(target3.view(-1).shape)
            print(predicted5.shape)
            print(target5.shape)
			'''


            B_metrics1.add_batch(predicted11.view(-1).cpu(), target11.view(-1).cpu())
            #B_metrics2.add_batch(predicted2.view(-1,data.num_relation_classes).cpu(), target2.view(-1,data.num_relation_classes).cpu())
            ...
            #loss_train = criterion(output.permute(0,2,1), target) + criterion2(output2,target2, target, opt.alpha_relation) #计算loss
            print("\n-----loss----: {}\t{}\n".format(loss_entity,criterion2(output2,target2, target, opt.alpha_relation)))
...
def val(model,model2,dataloader,num_relation_classes):
    ...
    for(input, target, target2) in t:
        ...
        predicted3 = torch.where(predicted2.int()==target2.int(), torch.tensor(1), torch.tensor(0))
        predicted3 = torch.sum(predicted3, dim=2)
        predicted3 = torch.where(predicted3.int()==num_relation_classes, torch.tensor(1), torch.tensor(0))
        target3 = torch.where(predicted3==predicted3, torch.tensor(1), torch.tensor(0))
...
def test():
    ...
    for (input, target, target2) in t:
        ...
        B_acc1.add_batch(predicted.view(-1).cpu(), target.view(-1).cpu())
        B_acc2.add_batch(predicted3.view(-1).cpu(), target3.view(-1).cpu())
    ...
    print("        acc")
    print("test1: {}".format(test_metrics1_print))
    print("test2: {}\n".format(test_metrics2_print))
